...
const App = () => {
  ...
  const [localtask, setlocalTask] = useState(() => {
    try {
      const localData = localStorage.getItem("NotesApp");
  ...
  const deleteNote = (idx) => {
    ...
    // console.log(localCopy[idx]);
    // localCopy.splice(idx, 1)
    // localStorage.setItem("NotesApp", JSON.stringify(localCopy))
  ...
  const handleSubmit = (e) => {
    ...
    const copyTask = [...localtask]
    copyTask.push({ title, data, date })

    localStorage.setItem("NotesApp", JSON.stringify(copyTask))
    console.log("String", JSON.stringify(copyTask))

    // const localData = JSON.parse(localStorage.getItem("NotesApp"))
    setlocalTask(copyTask)
    // console.log(copyTask);
    // console.log("Parsed", JSON.parse(localStorage.getItem("NotesApp")));
